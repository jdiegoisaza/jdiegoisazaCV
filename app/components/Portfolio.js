        ...
        <div className="grid sm:grid-cols-2 gap-6">
          {proyectos.map((proyecto, idx) => (
            <Reveal key={proyecto.titulo} delay={idx * 50}>
              <article className="h-full rounded-lg border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 p-6 shadow-sm hover:shadow-lg dark:shadow-none hover:border-blue-200 dark:hover:border-blue-800 hover:-translate-y-0.5 transition-all">
                <h3 className="font-semibold text-neutral-900 dark:text-neutral-100 mb-1">
                  {proyecto.titulo}
                </h3>
                <p className="text-sm text-blue-600 dark:text-blue-400 mb-3">{proyecto.subtitulo}</p>
                ...
                <div className="flex flex-wrap gap-2 mb-4">
                  {proyecto.stack.map((tech) => (
                    <span
                      key={tech}
                      className="text-xs bg-neutral-100 dark:bg-neutral-800 text-neutral-700 dark:text-neutral-300 rounded px-2 py-1"
                    >
                      {tech}
                    </span>
                  ))}
